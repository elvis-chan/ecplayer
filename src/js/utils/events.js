...
const eventsApi = function eventsApi(obj, action, name, rest) {
  if (!name) {
    return true;
  }
...
const triggerEvents = function triggerEvents(events, args, context, catchExceptionsForName) {
  let i = -1;
...
export const on = function on(name, callback, context) {
...
export function once(name, callback, context) {
  ...
  let count = 0;
  const self = this;
  const onceCallback = function onceCallback() {
    if (count++) {
      return;
    }
...
export default { on, once, off, trigger, triggerSafe };
